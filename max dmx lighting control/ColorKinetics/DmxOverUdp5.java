...
public class DmxOverUdp5 extends MaxObject {
	...
	private int packSize = 4096; //536 for CK
	...
	public DmxOverUdp5(Atom[] args) {
		...
		// pds-480ca
		post("UdpSendRaw New: sending to "+host+" on port "+port);	
		
		if(args[3].isInt())
				packSize = args[3].getInt();	 
				post("UdpSendRaw New: sending to "+host+" on port "+port+" packsize "+packSize);	
	...
	public void jit_matrix(String dmx) {
		...
		for (int i=0; i<dim[1]; i++) {
			...
			// 	if((j % 40) >= 20){
// 				reversi = (-1*(j % 40)+20);
// 				buffer[headerSize+((j+reversi)*3)] = (byte)row_r[j];
// 				buffer[headerSize+((j+reversi)*3)+1] = (byte)row_g[j];
// 				buffer[headerSize+((j+reversi)*3)+2] = (byte)row_b[j];
// 				}
// 				else{
// 				buffer[headerSize+(j*3)] = (byte)row_r[j];
// 				buffer[headerSize+(j*3)+1] = (byte)row_g[j];
